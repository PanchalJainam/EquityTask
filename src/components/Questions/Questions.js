...
const Questions = () => {
  ...
  //   const second = seconds.getSeconds();
  // };
  // setInterval(() => {
  //   timer()
  // }, 1000);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      const qid = "65755d3dd3b55322fd7b1d2c";
      const queResponse = await axios.get(
        `http://localhost:5000/api/quizquestions/quizquesions_get?id=${qid}`
      );
      console.log(queResponse.data);
      setQue(queResponse.data);
      try {
    ...
    <>
      <div className="px-28 py-10">
        ...
        <div className="py-5">
          ...
          {/* 
          <div className="w-full my-5 py-4 px-4 bg-white h-64  shadow-black/10 shadow-lg rounded-md">
            {items &&
              items.length > 0 &&
              items.map((ele, index) => {
                return (
            ...
            <div className=" px-2 py-1 w-full items-baseline">
              <button
